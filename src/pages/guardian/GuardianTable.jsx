...
import { Search, Plus, Eye, ChevronLeft, ChevronRight, FileUser } from "lucide-react";

const GuardianTable = ({ onAddClick, onViewClick , refreshTrigger}) => {
  ...
  useEffect(() => {
    const fetchGuardians = async () => {
      try {
        const response = await axios.get("http://localhost:8080/guardian/all");
  ...
  // --- 3. THE SEARCH ENGINE ---
  // Safely filter the real database records
  const filteredGuardians = guardians.filter((g) => {
    const searchLower = searchTerm.toLowerCase();
    const nameMatch = g.fullname && g.fullname.toLowerCase().includes(searchLower);
    const nicMatch = g.nic && g.nic.toLowerCase().includes(searchLower);
    const idMatch = g.id && g.id.toString().includes(searchLower);
  ...
  const currentData = filteredGuardians.slice(startIndex, startIndex + itemsPerPage);
          ...
          <tbody>
            {isLoading ? (
              ...
              currentData.map((guardian) => (
                ...
                >
                  {/* Formatted to look like GRD-001 */}
                  ...
                  <td className="px-6 py-4">
                    ...
                    {guardian.addDate ? guardian.addDate.split('T')[0] : 'N/A'}
                  </td>
                  <td className="px-6 py-4 tracking-wide">{guardian.nic || '-'}</td>
              ...
              <tr>
                <td colSpan="5" className="px-6 py-12 text-center text-gray-500 bg-gray-50/30">
                  <p className="text-lg font-medium">No guardians found.</p>
        ...
        <div className="p-4 border-t border-gray-200/60 bg-white/40 flex items-center justify-between">
          <div className="text-sm text-gray-500 font-medium">
            ...
            <span className="text-gray-900">
              {Math.min(startIndex + itemsPerPage, filteredGuardians.length)}
            </span>{" "}
            of <span className="text-gray-900">{filteredGuardians.length}</span> entries
          </div>
