...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    const payload = {
        username: username, // Your React state variable for the username input
        password: password  // Your React state variable for the password input
      };
    ...
    try {
      ...
      // Save ALL the data from the backend
      localStorage.setItem("pirivena_token", response.data.token);
      ...
      <div className="relative z-10 w-full max-w-md p-8 bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]">
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-5">
          <div className="flex flex-col">
            <label className="text-sm font-semibold text-indigo-100 mb-1 ml-1">
              Username
            </label>
            <input
              type="text"
